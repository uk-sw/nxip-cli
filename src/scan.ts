 ...
 * networks in one region. Every discovered network gets environment
 * "production", and the operator is told to change it if that is wrong.
 *
 * Earlier versions rewrote environment to each network's own name whenever a
 * region held more than one, to dodge the API's one-kind-tagged-subnet-per-key
 * rule. That could not work: this runs offline and never contacts the API, so
 * it invented an environment no pool had, and the manifest then referenced
 * pools that did not exist and raised overlap warnings that were untrue. The
 * rule it was avoiding was relaxed on 2026-09-14, so several VPCs in one
 * region now import side by side as they are. A scan cannot know which VPC is
 * staging, so the result is still a proposal to review.
 */
...
export interface ManifestOptions {
  ...
  includeDefaultNetworks?: boolean;
  /**
   * When the scan ran. Defaults to now; passed in so tests can pin it.
   * A manifest is a snapshot of a cloud estate that keeps changing, so the
   * file says how old its snapshot is rather than leaving the reader to
...
export function renderDiscoveryManifest(report: ScanReport, options: ManifestOptions = {}): string {
  ...
  const lines: string[] = [
    '# Generated by `npx nxip-cli scan --emit-manifest`.',
    // UTC, whole seconds: unambiguous across time zones, and sub-second
    // precision says nothing useful about a cloud estate.
    `# Generated at ${(options.generatedAt ?? new Date()).toISOString().replace(/\.\d{3}Z$/, 'Z')} (UTC).`,
    '#',
