...
describe('when a manifest was generated', () => {
  const report = () =>
    analyseDiscovery(discovery({ networks: [{ id: 'vpc-1', name: 'app', region: 'eu-west-2', cidrs: ['10.9.0.0/16'] }] }));

  it('says when, in UTC, right under the generated-by line', () => {
    const rendered = renderDiscoveryManifest(report(), { generatedAt: new Date('2026-09-15T14:02:11.482Z') });
    const lines = rendered.split('\n');
    expect(lines[0]).toBe('# Generated by `npx nxip-cli scan --emit-manifest`.');
    expect(lines[1]).toBe('# Generated at 2026-09-15T14:02:11Z (UTC).');
  });

  it('defaults to the time it was rendered', () => {
    const before = Date.now();
    const match = renderDiscoveryManifest(report()).match(/^# Generated at (\S+) \(UTC\)\.$/m);
